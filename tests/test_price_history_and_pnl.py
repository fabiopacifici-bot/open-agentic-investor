...
def test_fetch_volume_history_returns_volumes():
    rows = [
        ("NVDA", f"2025-02-{i:02d}", 500.0, 2000000 + i * 1000) for i in range(1, 16)
    ]
    db = _make_db_with_prices(rows)
    with patch("scripts.portfolio_manager.DB_PATH", db):
    ...
    assert len(vols) == 15
    assert (
        vols[0] == pytest_approx(2001000.0, rel=1e-6)
        if False
        else abs(vols[0] - 2001000.0) < 1
    )
...
def test_sell_suppressed_when_pnl_below_minus5():
    ...
    with (
        patch(
            "scripts.portfolio_manager.load_portfolio_config", return_value=config_mock
        ),
        patch(
            "scripts.portfolio_manager._build_indicator_reason", return_value=(None, "")
        ),
...
def test_indicator_sell_suppressed_when_pnl_below_minus5():
    ...
    config_mock = {
        "portfolio": [{"symbol": "NVDA", "buy_threshold": None, "sell_threshold": None}]
    }
    ...
    with (
        ...
        patch(
            "scripts.portfolio_manager._build_indicator_reason",
            return_value=("SELL", "RSI=75"),
        ),
...
def test_cache_skip_no_request_when_rows_exist():
    ...
    with (
        patch.dict(os.environ, {"ALPHA_API": "test_key", "INVESTMENTS_DB": db}),
...
def test_main_does_not_crash_when_threshold_is_none(capsys):
    """main() must not raise TypeError when a recommendation has threshold=None (indicator-only signal)."""
    from scripts.portfolio_manager import main
    ...
    with (
        patch(
            "scripts.portfolio_manager.analyze_portfolio", return_value=[indicator_rec]
        ),
        patch("scripts.fetch_prices.fetch_stock_prices", return_value={"NVDA": 500.0}),
        patch("scripts.fetch_account_info.fetch_account_info", return_value={}),
        patch("builtins.input", return_value="no"),
    ):
        main()  # Must not raise TypeError

    captured = capsys.readouterr()
    assert "Threshold: N/A" in captured.out
